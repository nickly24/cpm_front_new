...
export const homeworkChatApi={
  ...
  send:(homeworkId:number,text:string,studentId?:number)=>apiRequest<{id:number;thread_id:number}>("/api/homework-chat/messages",{method:"POST",body:JSON.stringify({homework_id:homeworkId,student_id:studentId,text,client_message_id:crypto.randomUUID()})}),
