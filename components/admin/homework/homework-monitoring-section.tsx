...
export function HomeworkMonitoringSection() {
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.list}>
      ...
      {data?.recent_jobs.filter((job) => job.status === "failed").map((job) => <article key={job.id}><div><h2>{job.error_code ?? "Ошибка"}</h2><p>{job.id} · попыток {job.attempts}</p></div>{job.manual_attempts < 3 ? <EditOnly><button onClick={() => void retry(job.id)}>Повторить</button></EditOnly> : null}</article>)}
    </div>
  </div>;
}
