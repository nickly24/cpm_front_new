...
export function ReviewQueueSection(){
  ...
  return <div className={styles.page}><header><div><span>Проверка</span><h1>Очередь домашних работ</h1><p>Активные работы, доступные вам для проверки. Оценённые находятся в архиве администратора.</p></div><div><button onClick={()=>void load()}>Обновить</button></div></header>{error?<div className={styles.error}>{error}</div>:null}<div className={styles.list}>{items.map(item=><article key={item.id}><div><h2>{item.student_name}</h2><p>{item.homework_name} · {item.group_name??"Без группы"}</p><span>{labels[item.state]??item.state}</span>{item.revision_comment?<p>{item.revision_comment}</p>:null}</div><div className={styles.actions}><button onClick={()=>void open(item)}>PDF</button>{item.state==="submitted"?<EditOnly><button className={styles.primary} disabled={busy===item.id} onClick={()=>void action(item,"claim")}>Взять</button></EditOnly>:null}{item.state==="in_review"?<>{isAdminCabinet(user?.role)&&(item.reviewer_id!==user?.id||item.reviewer_role!==user?.role)?<EditOnly><button onClick={()=>void action(item,"takeover")}>Перехватить</button></EditOnly>:null}<EditOnly><button onClick={()=>void action(item,"release")}>Освободить</button></EditOnly><EditOnly><button onClick={()=>{const message=window.prompt("Что нужно исправить?");if(message)void action(item,"request-revision",{message})}}>На доработку</button></EditOnly><EditOnly><button className={styles.primary} onClick={()=>grade(item)}>Оценить</button></EditOnly></>:null}</div></article>)}{!items.length?<p className={styles.empty}>Работ пока нет.</p>:null}</div></div>;
}
